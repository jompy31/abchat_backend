...
const CreateWhatsAppService = async ({ name, status = "OPENING", queueIds = [], greetingMessage, complationMessage, outOfHoursMessage, ratingMessage, isDefault = false, companyId, token = "", provider = "beta", timeSendQueue, sendIdQueue, promptId }) => {
    ...
    if (company !== null) {
        ...
        if (whatsappCount >= company.plan.connections) {
            throw new AppError_1.default(`Número máximo de conexiones ya alcanzadas: ${whatsappCount}`);
        }
    }
    const schema = Yup.object().shape({
        name: Yup.string()
            .required()
            .min(2)
            .test("Check-name", "Este nombre ya está siendo utilizado por otra conexión.", async (value) => {
            if (!value)
                return false;
    ...
    if (token !== null && token !== "") {
        const tokenSchema = Yup.object().shape({
            token: Yup.string()
                .required()
                .min(2)
                .test("Check-token", "Este token de WhatsApp ya está en uso.", async (value) => {
                if (!value)
                    return false;
