...
const sendMessageImage = async (wbot, contact, ticket, url, caption) => {
    ...
    try {
        ...
        sentMessage = await wbot.sendMessage(`${contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, {
            text: (0, Mustache_1.default)('No pude enviar el PDF, ¡inténtalo de nuevo!', contact)
        });
    }
...
const getBodyMessage = (msg) => {
    try {
        ...
        if (!objKey) {
            logger_1.logger.warn(`#### No encontré el tipo 152: ${type}
${JSON.stringify(msg)}`);
...
const isValidMsg = (msg) => {
    ...
    try {
        ...
        if (!ifType) {
            logger_1.logger.warn(`#### No encontré el tipo en isValidMsg: ${msgType}
