...
const CreateUserService = async ({ email, password, name, queueIds = [], companyId, profile = "admin", whatsappId }) => {
    if (companyId !== undefined) {
        ...
        if (company !== null) {
            ...
            if (usersCount >= company.plan.users) {
                throw new AppError_1.default(`Número máximo de usuarios ya alcanzado: ${usersCount}`);
            }
        }
    }
    const schema = Yup.object().shape({
        ...
        email: Yup.string()
            .email()
            .required()
            .test("Check-email", "Ya existe un usuario con este correo electrónico.", async (value) => {
            if (!value)
                return false;
