                        ...
                        if (!isNaN(idQueue) && Number.isInteger(idQueue) && !isNaN(timeQueue) && Number.isInteger(timeQueue)) {
                            ...
                            if (count > 0) {
                                tickets.map(async (ticket) => {
                                    ...
                                    //   ticket,
                                    // });
                                    logger_1.logger.info(`Servicio perdido: ${ticket.id} - Empresa: ${companyId}`);
                                });
                            }
                            else {
                                logger_1.logger.info(`No se encontró ningún servicio perdido - Empresa: ${companyId}`);
                            }
                        }
                        else {
                            logger_1.logger.info(`Condición no respetada - Empresa: ${companyId}`);
                        }
                    }
                }
            });
    ...
    try {
        ...
        if (count > 0) {
            schedules.map(async (schedule) => {
                ...
                logger_1.logger.info(`Envió previsto para: ${schedule.contact.name}`);
            });
        }
    }
    catch (e) {
...
async function handleSendScheduledMessage(job) {
    ...
    try {
        ...
        Sentry.captureException(e);
        logger_1.logger.info(`Error al intentar consultar el horario: ${schedule.id}`);
    }
    try {
...
async function handleVerifyCampaigns(job) {
    ...
    for (let campaign of campaigns) {
        try {
            ...
            logger_1.logger.info(`Campaña enviada a la cola de procesamiento: Campanha=${campaign.id}, Delay Inicial=${delay}`);
        ...
        if (campaign) {
            ...
            if ((0, lodash_1.isArray)(contacts)) {
                ...
                for (let i = 0; i < contactData.length; i++) {
                    ...
                    logger_1.logger.info(`Registro enviado a la cola de activación: Campaña=${campaign.id};Contacto=${contacts[i].name};delay=${delay}`);
                }
...
async function handleDispatchCampaign(job) {
    try {
        ...
        logger_1.logger.info(`Activador de campaña solicitado: Campaña=${campaignId};Registro=${campaignShippingId}`);
        ...
        logger_1.logger.info(`Campaña enviada a: Campaña=${campaignId};Contacto=${campaignShipping.contact.name}`);
    }
    catch (err) {
        Sentry.captureException(err);
...
async function handleInvoiceCreate() {
    logger_1.logger.info("Iniciar generación de facturas");
...
async function startQueueProcess() {
    logger_1.logger.info("Iniciando procesamiento de cola");
